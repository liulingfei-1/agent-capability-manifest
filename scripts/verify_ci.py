#!/usr/bin/env python3
"""CI verification: run FIX-005/006 + CL-ADV runners, check verdicts all-pass and canonical digests.
...
def run_runner(name, runner, fixture, expect_input_prefix):
    ...
    if expect_input_prefix and not inp.startswith(expect_input_prefix):
        failures.append(f"{name}: input_digest {inp[:16]} != expected prefix {expect_input_prefix}")
...
# CL-ADV 001..005 (claims_lookup adversarial, Stone line)
import glob as _glob
for i in range(1, 6):
    matches = _glob.glob(os.path.join(BASE, f"fixtures/CL-ADV-00{i}_*.json"))
    if not matches:
        failures.append(f"CL-ADV-00{i}: fixture not found")
        continue
    run_runner(f"CL-ADV-00{i}", "runners/cl_adv_runner.py", matches[0], None)
